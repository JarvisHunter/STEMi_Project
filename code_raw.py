...
start_rect.center=(screen_w/2,400)
...
ins_rect=pg.Rect(screen_w/2 - ins_w/2, 550 - ins_h/2,ins_w,ins_h)
...
cat_image=pg.image.load('sources/cat.png').convert_alpha()
cat_image=Scale(cat_image,4)
cat_choose=-1
(cat_w,cat_h)=getsize(cat_image)
Cat=(cat_image,pg.Rect(0,0,cat_w,cat_h))
...
#reset
def reset():
    global ticks,life,Cat,cat_speed,points,Trash_temp,Bin_images
    ticks=0
    life=life_limit
    Trash_temp.clear()
    Cat[1].center=cat_sleep_pos
...
def intro():
    ...
    if start_mask==0:
        DisplayImage(intro_start,screen_w/2,400)
    else: DisplayImage(intro_start2,screen_w/2,400)
    if ins_mask==0:
        DisplayImage(intro_ins,screen_w/2,550)
    else: DisplayImage(intro_ins2,screen_w/2,550)
    pg.display.update()
...
def main_game():
    global ticks,cat_speed,cat_sleep,cat_signal,seconds,Trash_tmp_choose,Bin_tmp_choose,cat_choose,cat_sleep_timestamp,state,cat_aim,hints_choose,life,life,points,Cat
    global cat_speed_a,cat_speed_frequency
        ...
        if event.type==pg.MOUSEBUTTONDOWN:
            ...
            if cat_choose==-1:
                if is_over(Cat[1],pos):
                    ...
                    cat_sleep=np.random.randint(1,8)
                    random_aim=np.random.randint(0,3)
                    cat_aim=Bin_images[random_aim][2].center
    ...
    for Bin in Bin_images:
        if is_rect_over(Bin[2],Cat[1]) and Bin_tmp_choose!=Bin[1]:
            points[Bin[1]]=0
    ...
    screen.blit(Cat[0],(Cat[1].x,Cat[1].y))
...
img_set=load_images('trash_2')
img=[]
for tmp in img_set:
    img.append(img_set[tmp])
main=pg.image.load('screen_main.png')
while (True):
    screen.blit(main,(0,0))
    cnt=0
    for image in img:
        cnt+=1
        DisplayImage(image,200+300*(cnt%5),200+150*(cnt/5))
    for event in pg.event.get():
...
# Finally, This is the dusk of a wonderful code ~~~
# 

# Author: NguyenCaoMinhTuan aka JarvisHunter.
